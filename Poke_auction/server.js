...
function normalizeString(str) { return str.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim(); }
...
function getShopItems() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    let sum = 0;
    for (let item of SHOP_ITEMS) { sum += item.weight; if (rand <= sum) { items.push(item); break; } }
  }
  return items;
}
...
io.on('connection', (socket) => {
  ...
  socket.on('submitImpWord', (word) => {
    ...
    if (room.impSettings.mode === 'undercover' && socket.id === room.impState.imposteurId) forbiddenName = normalizeString(room.impState.undercoverPoke.name);
    if (submittedWord.includes(forbiddenName) || (forbiddenName.includes(submittedWord) && submittedWord.length > 3)) {
      ...
      if (room.impState.timeLeft <= 0) { clearInterval(room.impState.timer); acceptWordAndNextTurn(currentRoom, activePlayerId, "⏳ (Temps écoulé)", true); }
      return;
    }
    clearInterval(room.impState.timer);
    acceptWordAndNextTurn(currentRoom, activePlayerId, word, false);
  });
  socket.on('submitImpVote', (suspectId) => { const room = rooms[currentRoom]; if (!room || room.state !== 'VOTING') return; room.impState.votes[socket.id] = suspectId; const totalPlayers = Object.keys(room.players).filter(id => room.players[id].connected).length; if (Object.keys(room.impState.votes).length >= totalPlayers) resolveVoting(currentRoom); });
...
async function startNextAuction(roomCode) { 
    ...
    r.auctionTimer = setInterval(() => { 
        ...
        if(r.currentAuction.timeLeft <= 0) { clearInterval(r.auctionTimer); endAuction(roomCode); } 
    }, 1000); 
...
async function checkAndFillTeams(roomCode) { 
    ...
    if(p1.team.length >= 3 && p2.team.length >= 3) { r.state = 'TRANSITIONING_TO_SHOP'; return setTimeout(() => enterShopPhase(roomCode), 2000); } 
    let n = null; if(p1.team.length >= 3 && p2.team.length < 3) n = p2; else if(p2.team.length >= 3 && p1.team.length < 3) n = p1; else if(p1.budget < 50 && p2.budget < 50) { if(p1.team.length < 3) n = p1; else if(p2.team.length < 3) n = p2; } 
    if(n) { const pk = await getRandomPokemon(); if(pk) n.team.push(pk); io.to(roomCode).emit('auctionEnded', { players: r.players, winnerName: "Système", pokemon: `${pk?.name} (Auto-Fill)` }); setTimeout(() => checkAndFillTeams(roomCode), 1500); } 
    else { setTimeout(() => startNextAuction(roomCode), 2000); } 
}
...
function resolveTurn(roomCode) { 
    const r = rooms[roomCode]; const b = r.battleState; b.lastDamage = null; const aP = r.players[b.attackerId]; const dP = r.players[b.defenderId]; const aIdx = b.attackerId === b.p1.id ? b.p1ActiveIndex : b.p2ActiveIndex; const dIdx = b.defenderId === b.p1.id ? b.p1ActiveIndex : b.p2ActiveIndex; const aPk = aP.team[aIdx]; const dPk = dP.team[dIdx]; 
    if(dPk.item && dPk.item.id === 'poudre' && Math.random() < 0.15) { b.attackerAction = null; b.defenderAction = null; sendBattleUpdate(roomCode, `${dPk.name} esquive l'attaque grâce à Poudre Claire !`); triggerBotBattle(roomCode); return; } 
    let rA = b.attackerAction === 'special' ? aPk.spAtk : aPk.attack; let rD = b.defenderAction === 'specialDef' ? dPk.spDef : dPk.def; 
    if(aPk.item && aPk.item.id === 'bandeau' && b.attackerAction === 'physique') rA *= 1.2; if(aPk.item && aPk.item.id === 'lunettes' && b.attackerAction === 'special') rA *= 1.2; if(aPk.item && aPk.item.id === 'orbe') rA *= 1.3; if(dPk.item && dPk.item.id === 'veste') { if(b.defenderAction === 'specialDef') rD *= 1.3; if(b.defenderAction === 'physiqueDef') rD *= 0.9; } 
    let dmg = Math.max(5, Math.floor(rA - (rD / 3))); b.lastDamage = { targetId: b.defenderId, amount: dmg }; let log = `${aPk.name} inflige ${dmg} dégâts !`; dPk.hp -= dmg; if(dPk.hp <= 0 && dPk.item && dPk.item.id === 'ceinture') { dPk.hp = 1; dPk.item = null; log += ` Survit grâce à Ceinture Force !`; } if(dPk.hp < 0) dPk.hp = 0; if(aPk.item && aPk.item.id === 'orbe') { const rc = Math.floor(aPk.hpMax * 0.1); aPk.hp = Math.max(0, aPk.hp - rc); log += ` Orbe Vie draine ${rc} PV.`; } if(aPk.item && aPk.item.id === 'grelot') { const hl = Math.floor(dmg * 0.2); aPk.hp = Math.min(aPk.hpMax, aPk.hp + hl); log += ` Grelot soigne ${hl} PV.`; } if(dPk.item && dPk.item.id === 'casque' && b.attackerAction === 'physique' && dPk.hp > 0) { aPk.hp = Math.max(0, aPk.hp - 15); log += ` Casque Brut inflige 15 PV !`; } if(dPk.item && dPk.item.id === 'sitrus' && dPk.hp > 0 && dPk.hp <= dPk.hpMax / 2) { dPk.hp = Math.min(dPk.hpMax, dPk.hp + 30); dPk.item = null; log += ` Baie Sitrus restaure 30 PV !`; } let fS = null; if(dPk.hp > 0 && dPk.item && dPk.item.id === 'fuite') { dPk.item = null; fS = b.defenderId; log += ` Bouton Fuite activé !`; } else if(dPk.hp > 0 && aPk.item && aPk.item.id === 'cartouche') { aPk.item = null; fS = b.defenderId; log += ` Cartouche Rouge activée !`; } 
    const nA = getFirstAliveIndex(aP.team); const nD = fS === b.defenderId ? getFirstAliveIndex(dP.team.filter((x,i)=> i!==dIdx && x.hp>0)) : getFirstAliveIndex(dP.team); let tNd = fS === b.defenderId && dP.team.findIndex((x,i)=> i!==dIdx && x.hp>0) !== -1 ? dP.team.findIndex((x,i)=> i!==dIdx && x.hp>0) : getFirstAliveIndex(dP.team); 
    if(nA === -1 || tNd === -1) { r.state = 'GAME_OVER'; log += nA === -1 ? ` ${dP.name} gagne !` : ` ${aP.name} gagne !`; } 
    else { if(aPk.hp > 0 && aPk.item && aPk.item.id === 'restes') aPk.hp = Math.min(aPk.hpMax, aPk.hp + 10); if(dPk.hp > 0 && dPk.item && dPk.item.id === 'restes') dPk.hp = Math.min(dPk.hpMax, dPk.hp + 10); if(b.attackerId === b.p1.id) { b.p1ActiveIndex = nA; b.p2ActiveIndex = tNd; } else { b.p2ActiveIndex = nA; b.p1ActiveIndex = tNd; } const t = b.attackerId; b.attackerId = b.defenderId; b.defenderId = t; } 
...
async function startImposteurRound(roomCode) {
  ...
  let poke2 = null;
  if (room.impSettings.mode === 'undercover') { poke2 = await getRandomPokemon(); failSafe = 0; while((!poke2 || poke2.id === poke.id) && failSafe < 5) { poke2 = await getRandomPokemon(); failSafe++; } }
  if (!poke || (room.impSettings.mode === 'undercover' && !poke2)) { io.to(roomCode).emit('errorMsg', "Erreur API Pokémon. Relancez."); room.state = 'LOBBY'; return; }
  
  room.impState.secretPoke = poke; room.impState.undercoverPoke = poke2; room.impState.wordsLog = []; room.impState.votes = {}; room.impState.currentWordLap = 1;
  ...
  playerIds.forEach(id => {
    ...
    let sentPoke = poke; let flagImp = isImp;
    if (room.impSettings.mode === 'undercover') { flagImp = false; sentPoke = isImp ? poke2 : poke; } else { sentPoke = isImp ? null : poke; }
...
function startImposteurVoting(roomCode) { const room = rooms[roomCode]; room.state = 'VOTING'; io.to(roomCode).emit('impStartVoting', { log: room.impState.wordsLog }); }
function resolveVoting(roomCode) { const room = rooms[roomCode]; room.state = 'RESOLUTION'; const counts = {}; Object.values(room.impState.votes).forEach(id => counts[id] = (counts[id] || 0) + 1); let accusedId = null; let maxVotes = 0; for (const [id, count] of Object.entries(counts)) { if (count > maxVotes) { maxVotes = count; accusedId = id; } } const impId = room.impState.imposteurId; const imposteurCaught = (accusedId === impId); io.to(roomCode).emit('impVoteResult', { votes: room.impState.votes, accusedId, imposteurCaught, realImposteurId: impId }); if (imposteurCaught) { room.state = 'COUNTER_ATTACK'; io.to(roomCode).emit('impCounterAttackPhase', { imposteurId: impId }); room.impState.timer = setTimeout(() => { finishImposteurRound(roomCode, false); }, 45000); } else { finishImposteurRound(roomCode, false); } }
function finishImposteurRound(roomCode, counterAttackSuccess, guess = null) { const room = rooms[roomCode]; room.state = 'ROUND_END'; let winners = []; const impId = room.impState.imposteurId; if (room.state === 'RESOLUTION' || (room.state === 'ROUND_END' && !counterAttackSuccess)) { if (counterAttackSuccess) { winners = [impId]; } else if (guess !== null) { winners = Object.keys(room.players).filter(id => id !== impId); } else { winners = [impId]; } } winners.forEach(id => { if (room.players[id]) room.players[id].score++; }); io.to(roomCode).emit('impRoundEnd', { secretPoke: room.impState.secretPoke, winners, scores: room.players, guess }); setTimeout(() => { if (room.impState.round < room.impSettings.maxRounds) { room.impState.round++; startImposteurRound(roomCode); } else { room.state = 'LOBBY'; io.to(roomCode).emit('impGameOver', { scores: room.players }); } }, 10000); }
