...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', (userData) => {
    ...
    rooms[roomCode] = { 
      ...
      guessState: { board: [], secrets: {}, turn: null, chat: [] }
    };
  ...
  socket.on('joinRoom', ({ roomCode, name, avatar }) => {
    ...
    if (disconnectedPlayer) {
      ...
      if (room.guessState && room.guessState.secrets[oldId]) {
        room.guessState.secrets[socket.id] = room.guessState.secrets[oldId];
        delete room.guessState.secrets[oldId];
        if (room.guessState.turn === oldId) room.guessState.turn = socket.id;
      }
      ...
      } else if (room.gameType === 'imposteur' || room.gameType === 'guess') {
    ...
    const role = ((room.gameType === 'pokeauc' || room.gameType === 'guess') && activePlayers >= 2) ? 'spectator' : 'player';
    ...
    if (currentRoom && rooms[currentRoom]) {
      ...
      if (room.players[socket.id]) {
        ...
        if (isPlayer) {
          ...
          else if (room.gameType === 'imposteur' || room.gameType === 'guess') io.to(currentRoom).emit('lobbyUpdate', { players: room.players, host: room.host });
        }
      }
    }
  });
  ...
  socket.on('startGuessGame', async () => {
    ...
    players.forEach(p => {
      room.guessState.secrets[p.id] = board[Math.floor(Math.random() * board.length)];
    });

    io.to(currentRoom).emit('guessGameStarted', {
      board: board,
      secrets: room.guessState.secrets,
      turn: room.guessState.turn,
...
async function startNextAuction(roomCode) { 
  ...
  r.auctionTimer = setInterval(() => { 
    ...
    if(r.currentAuction.timeLeft <= 0) { 
      clearInterval(r.auctionTimer); 
      endAuction(roomCode); 
    } 
  }, 1000); 
  triggerBotAuction(roomCode); 
...
async function checkAndFillTeams(roomCode) { 
  ...
  if(p1.team.length >= 3 && p2.team.length >= 3) { 
    r.state = 'TRANSITIONING_TO_SHOP'; 
    return setTimeout(() => enterShopPhase(roomCode), 2000); 
  } 
  
  let n = null; 
  if(p1.team.length >= 3 && p2.team.length < 3) n = p2; 
  else if(p2.team.length >= 3 && p1.team.length < 3) n = p1; 
  else if(p1.budget < 50 && p2.budget < 50) { 
    if(p1.team.length < 3) n = p1; 
    else if(p2.team.length < 3) n = p2; 
  } 
...
function sendBattleUpdate(roomCode, logMsg) { 
  const r = rooms[roomCode]; 
  io.to(roomCode).emit('battleUpdate', { battle: r.battleState, players: r.players, log: logMsg, gameState: r.state }); 
}

function sendBattleUpdateToSocket(sId, roomCode, logMsg) { 
  const r = rooms[roomCode]; 
  io.to(sId).emit('battleUpdate', { battle: r.battleState, players: r.players, log: logMsg, gameState: r.state }); 
}
